...
% Use classic BibTeX instead of biblatex via class option [classicbib]
\documentclass[11pt,en,classicbib]{elegantpaper}
...
% Using BibTeX backend now (see documentclass option above)
% \addbibresource[location=local]{refs.bib} % reference file (biblatex) — disabled
\begin{document}
...
This template uses biblatex to generate the bibliography, the default citestyle and bibliography style are both \lstinline{numeric}. 
Let's take a glance at the citation effect. TEST ~\cite{Kong24isrr_submitted} use data from a major peer-to-peer lending TEST \cite{Silver10nips} marketplace in China to study whether female and male investors evaluate loan performance differently TEST \cite{Zhitnikov22ai}. 
